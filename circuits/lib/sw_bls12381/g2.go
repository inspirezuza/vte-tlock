...
//
// โ… p can be equal to q, and either or both can be (0,0).
...
func (g2 *G2) AddUnified(p, q *G2Affine) *G2Affine {
	...
	// ฮป = ((p.x+q.x)ยฒ - p.x*q.x + a)/(p.y + q.y)
...
func (g2 G2) add(p, q *G2Affine) *G2Affine {
	...
	// compute ฮป = (q.y-p.y)/(q.x-p.x)
	qypy := g2.Ext2.Sub(&q.P.Y, &p.P.Y)
	qxpx := g2.Ext2.Sub(&q.P.X, &p.P.X)
	ฮป := g2.Ext2.DivUnchecked(qypy, qxpx)

	// xr = ฮปยฒ-p.x-q.x
	xr0 := g2.fp.Eval([][]*baseEl{{&ฮป.A0, &ฮป.A0}, {mone, &ฮป.A1, &ฮป.A1}, {mone, &p.P.X.A0}, {mone, &q.P.X.A0}}, []int{1, 1, 1, 1})
	xr1 := g2.fp.Eval([][]*baseEl{{&ฮป.A0, &ฮป.A1}, {mone, &p.P.X.A1}, {mone, &q.P.X.A1}}, []int{2, 1, 1})
	xr := &fields_bls12381.E2{A0: *xr0, A1: *xr1}

	// p.y = ฮป(p.x-r.x) - p.y
	yr := g2.Ext2.Sub(&p.P.X, xr)
	yr0 := g2.fp.Eval([][]*baseEl{{&ฮป.A0, &yr.A0}, {mone, &ฮป.A1, &yr.A1}, {mone, &p.P.Y.A0}}, []int{1, 1, 1})
	yr1 := g2.fp.Eval([][]*baseEl{{&ฮป.A0, &yr.A1}, {&ฮป.A1, &yr.A0}, {mone, &p.P.Y.A1}}, []int{1, 1, 1})
...
func (g2 *G2) double(p *G2Affine) *G2Affine {
	...
	// compute ฮป = (3p.xยฒ)/2*p.y
	xx3a := g2.Square(&p.P.X)
	xx3a = g2.MulByConstElement(xx3a, big.NewInt(3))
	y2 := g2.Double(&p.P.Y)
	ฮป := g2.DivUnchecked(xx3a, y2)

	// xr = ฮปยฒ-2p.x
	xr0 := g2.fp.Eval([][]*baseEl{{&ฮป.A0, &ฮป.A0}, {mone, &ฮป.A1, &ฮป.A1}, {mone, &p.P.X.A0}}, []int{1, 1, 2})
	xr1 := g2.fp.Eval([][]*baseEl{{&ฮป.A0, &ฮป.A1}, {mone, &p.P.X.A1}}, []int{2, 2})
	xr := &fields_bls12381.E2{A0: *xr0, A1: *xr1}
...
func (g2 G2) triple(p *G2Affine) *G2Affine {
	...
	// compute ฮป1 = (3p.xยฒ)/2p.y
	xx := g2.Square(&p.P.X)
	xx = g2.MulByConstElement(xx, big.NewInt(3))
	y2 := g2.Double(&p.P.Y)
	ฮป1 := g2.DivUnchecked(xx, y2)

	// x2 = ฮป1ยฒ-2p.x
	x20 := g2.fp.Eval([][]*baseEl{{&ฮป1.A0, &ฮป1.A0}, {mone, &ฮป1.A1, &ฮป1.A1}, {mone, &p.P.X.A0}}, []int{1, 1, 2})
	x21 := g2.fp.Eval([][]*baseEl{{&ฮป1.A0, &ฮป1.A1}, {mone, &p.P.X.A1}}, []int{2, 2})
	x2 := &fields_bls12381.E2{A0: *x20, A1: *x21}

	// omit y2 computation, and
	// compute ฮป2 = 2p.y/(x2 โ’ p.x) โ’ ฮป1.
	x1x2 := g2.Sub(&p.P.X, x2)
	ฮป2 := g2.DivUnchecked(y2, x1x2)
	ฮป2 = g2.Sub(ฮป2, ฮป1)

	// compute x3 =ฮป2ยฒ-p.x-x2
	x30 := g2.fp.Eval([][]*baseEl{{&ฮป2.A0, &ฮป2.A0}, {mone, &ฮป2.A1, &ฮป2.A1}, {mone, &p.P.X.A0}, {mone, x20}}, []int{1, 1, 1, 1})
	x31 := g2.fp.Eval([][]*baseEl{{&ฮป2.A0, &ฮป2.A1}, {mone, &p.P.X.A1}, {mone, x21}}, []int{2, 1, 1})
	x3 := &fields_bls12381.E2{A0: *x30, A1: *x31}

	// compute y3 = ฮป2*(p.x - x3)-p.y
	y3 := g2.Ext2.Sub(&p.P.X, x3)
	y30 := g2.fp.Eval([][]*baseEl{{&ฮป2.A0, &y3.A0}, {mone, &ฮป2.A1, &y3.A1}, {mone, &p.P.Y.A0}}, []int{1, 1, 1})
	y31 := g2.fp.Eval([][]*baseEl{{&ฮป2.A0, &y3.A1}, {&ฮป2.A1, &y3.A0}, {mone, &p.P.Y.A1}}, []int{1, 1, 1})
...
func (g2 G2) doubleAndAdd(p, q *G2Affine) *G2Affine {
	...
	// compute ฮป1 = (q.y-p.y)/(q.x-p.x)
	yqyp := g2.Ext2.Sub(&q.P.Y, &p.P.Y)
	xqxp := g2.Ext2.Sub(&q.P.X, &p.P.X)
	ฮป1 := g2.Ext2.DivUnchecked(yqyp, xqxp)

	// compute x2 = ฮป1ยฒ-p.x-q.x
	x20 := g2.fp.Eval([][]*baseEl{{&ฮป1.A0, &ฮป1.A0}, {mone, &ฮป1.A1, &ฮป1.A1}, {mone, &p.P.X.A0}, {mone, &q.P.X.A0}}, []int{1, 1, 1, 1})
	x21 := g2.fp.Eval([][]*baseEl{{&ฮป1.A0, &ฮป1.A1}, {mone, &p.P.X.A1}, {mone, &q.P.X.A1}}, []int{2, 1, 1})
	x2 := &fields_bls12381.E2{A0: *x20, A1: *x21}

	// omit y2 computation
	// compute -ฮป2 = ฮป1+2*p.y/(x2-p.x)
	ypyp := g2.Ext2.Add(&p.P.Y, &p.P.Y)
	x2xp := g2.Ext2.Sub(x2, &p.P.X)
	ฮป2 := g2.Ext2.DivUnchecked(ypyp, x2xp)
	ฮป2 = g2.Ext2.Add(ฮป1, ฮป2)

	// compute x3 = (-ฮป2)ยฒ-p.x-x2
	x30 := g2.fp.Eval([][]*baseEl{{&ฮป2.A0, &ฮป2.A0}, {mone, &ฮป2.A1, &ฮป2.A1}, {mone, &p.P.X.A0}, {mone, x20}}, []int{1, 1, 1, 1})
	x31 := g2.fp.Eval([][]*baseEl{{&ฮป2.A0, &ฮป2.A1}, {mone, &p.P.X.A1}, {mone, x21}}, []int{2, 1, 1})
	x3 := &fields_bls12381.E2{A0: *x30, A1: *x31}

	// compute y3 = -ฮป2*(x3 - p.x)-p.y
...
func (g2 G2) doubleAndAddSelect(b frontend.Variable, p, q *G2Affine) *G2Affine {
	...
	// compute -ฮป2 = ฮป1+2*t.y/(x2-t.x)
	ypyp := g2.Ext2.Add(&t.P.Y, &t.P.Y)
	x2xp := g2.Ext2.Sub(x2, &t.P.X)
	ฮป2 := g2.Ext2.DivUnchecked(ypyp, x2xp)
	ฮป2 = g2.Ext2.Add(ฮป1, ฮป2)

	// compute x3 = (-ฮป2)ยฒ-t.x-x2
	x30 := g2.fp.Eval([][]*baseEl{{&ฮป2.A0, &ฮป2.A0}, {mone, &ฮป2.A1, &ฮป2.A1}, {mone, &t.P.X.A0}, {mone, x20}}, []int{1, 1, 1, 1})
	x31 := g2.fp.Eval([][]*baseEl{{&ฮป2.A0, &ฮป2.A1}, {mone, &t.P.X.A1}, {mone, x21}}, []int{2, 1, 1})
	x3 := &fields_bls12381.E2{A0: *x30, A1: *x31}

	// compute y3 = -ฮป2*(x3 - t.x)-t.y
	y3 := g2.Ext2.Sub(x3, &t.P.X)
	y30 := g2.fp.Eval([][]*baseEl{{&ฮป2.A0, &y3.A0}, {mone, &ฮป2.A1, &y3.A1}, {mone, &t.P.Y.A0}}, []int{1, 1, 1})
	y31 := g2.fp.Eval([][]*baseEl{{&ฮป2.A0, &y3.A1}, {&ฮป2.A1, &y3.A0}, {mone, &t.P.Y.A1}}, []int{1, 1, 1})
	y3 = &fields_bls12381.E2{A0: *y30, A1: *y31}
...
func (g2 *G2) computeTwistEquation(Q *G2Affine) (left, right *fields_bls12381.E2) {
	// Twist: Yยฒ == Xยณ + aX + b, where a=0 and b=4(1+u)
	// (X,Y) โ {Yยฒ == Xยณ + aX + b} U (0,0)
...
//
// โ ๏ธ  p must not be (0,0) and s must not be 0, unless [algopts.WithCompleteArithmetic] option is set.
...
//
// โ ๏ธ  The scalar s must be nonzero and the point Q different from (0,0) unless [algopts.WithCompleteArithmetic] is set.
// (0,0) is not on the curve but we conventionally take it as the
// neutral/infinity point as per the [EVM].
//
...
func (g2 *G2) scalarMulGLV(Q *G2Affine, s *Scalar, opts ...algopts.AlgebraOption) *G2Affine {
	...
	// We use the endomorphism ร  la GLV to compute [s]Q as
	// 		[s1]Q + [s2]ฮฆ(Q)
	...
	// flags sdBits to negate the points Q, ฮฆ(Q) instead of the corresponding
	// sub-scalars.
	...
	// i = 0
	// subtract the Q, ฮฆ(Q) if the first bits are 0.
