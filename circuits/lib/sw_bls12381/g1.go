...
func (g1 *G1) double(p *G1Affine) *G1Affine {
	...
	ฮป := g1.curveF.Div(xx3a, y1)

	// xr = ฮปยฒ-2p.x
	xr := g1.curveF.Eval([][]*baseEl{{ฮป, ฮป}, {mone, &p.X}}, []int{1, 2})

	// yr = ฮป(p-xr) - p.y
	yr := g1.curveF.Eval([][]*baseEl{{ฮป, g1.curveF.Sub(&p.X, xr)}, {mone, &p.Y}}, []int{1, 1})
...
func (g1 G1) add(p, q *G1Affine) *G1Affine {
	...
	ฮป := g1.curveF.Div(qypy, qxpx)

	// xr = ฮปยฒ-p.x-q.x
	xr := g1.curveF.Eval([][]*baseEl{{ฮป, ฮป}, {mone, g1.curveF.Add(&p.X, &q.X)}}, []int{1, 1})
...
func (g1 G1) doubleAndAdd(p, q *G1Affine) *G1Affine {
	...
	ฮป1 := g1.curveF.Div(yqyp, xqxp)

	// compute x1 = ฮป1ยฒ-p.x-q.x
	x2 := g1.curveF.Eval([][]*baseEl{{ฮป1, ฮป1}, {mone, g1.curveF.Add(&p.X, &q.X)}}, []int{1, 1})
	...
	ฮป2 := g1.curveF.Div(ypyp, x2xp)
	ฮป2 = g1.curveF.Add(ฮป1, ฮป2)

	// compute x3 = (-ฮป2)ยฒ-p.x-x2
	x3 := g1.curveF.Eval([][]*baseEl{{ฮป2, ฮป2}, {mone, &p.X}, {mone, x2}}, []int{1, 1, 1})

	// compute y3 = -ฮป2*(x3- p.x)-p.y
	y3 := g1.curveF.Eval([][]*baseEl{{ฮป2, g1.curveF.Sub(x3, &p.X)}, {mone, &p.Y}}, []int{1, 1})
...
func (g1 *G1) computeCurveEquation(P *G1Affine) (left, right *baseEl) {
	// Curve: Yยฒ == Xยณ + aX + b, where a=0 and b=4
